...
public class WaterWaveLoadingView extends View {
	...
	public void setmResourceBg(int mResourceBg) {
		this.mResourceBg = mResourceBg;
	}
	...
	public WaterWaveLoadingView(Context context, int mResourceBg, int mFillColor) {
		super(context);
		this.mResourceBg = mResourceBg;
		this.mFillColor = mFillColor;
	}
	 ...
	 * @param defStyle
	 */
	public WaterWaveLoadingView(Context context, AttributeSet attrs, int defStyle) {
	...
	private void init() {
		...
		paint.setColor(Color.parseColor("#00b5e8"));
		// 获得资源文件
		bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.bg_bottole);
		bitmap = ViewUtils.zoomImage(bitmap, bitmap.getWidth() / 2, bitmap.getHeight() / 2);
		// 设置宽高为图片的宽高
	...
	private void drawTargetBitmap() {
		...
		path.cubicTo(controlX / 2, waveY - (controlY - waveY), (controlX + width) / 2, controlY, width, waveY);
		// 与下下边界闭合
