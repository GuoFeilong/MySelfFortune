...
public class FlowWaterSurfaceView extends SurfaceView implements SurfaceHolder.Callback, SensorEventListener {
	...
	public static interface IWaterStateListener {
		...
		public void onWaterLeverChange(float x, float y);
	...
	public static interface IWaterChangedListener {
		/**
		 * 当水向左倾斜90度以上调用
		 */
		public void onWaterLeansChanged(float angle);
	...
	private IWaterChangedListener mWaterChangedListener;
	...
	private void init() {
		mBackgroundColor = getResources().getColor(R.color.conmmd_background);
		...
		WATER_TOP = resources.getDimension(R.dimen.homepage_bottle_fullline);
		mFullLine = mHeight / 2;
	...
	private int WATER_BOTTOM = ViewUtils.dpToPx(getResources(), 20);
	...
	public void refreshWater(float per) {
		if (per == 0) {
			...
			mBaseLine = mHeight - (mHeight - mFullLine) * (per) - WATER_BOTTOM;
		}
		// TODO
		if (mWaterStateListener != null) {
			mWaterStateListener.onWaterLeverChange(mWidth - ((mWidth + (mBaseLine >= mHeight - WATER_BOTTOM ? mHeight - WATER_BOTTOM : mBaseLine)) / 3) - ViewUtils.dpToPx(getResources(), 20), mBaseLine >= mHeight - WATER_BOTTOM ? mHeight
					- WATER_BOTTOM : mBaseLine);
		}
		mIsInit = false;
		mPer = 0;
		mAngle = 0;
		mLoaclPerData = getFloatFormate(per, 2);
		mUpOrDownNum = Math.abs((mLoaclPerData >= 0 ? mLoaclPerData : 0) - (mIsInit ? mPer : 1)) / (WATERLEVEL_TIME / SLEEP_TIME);
	}
	...
	public void setWaterPer(float per) {
		...
		// TODO
		if (mWaterStateListener != null) {
			mWaterStateListener.onWaterLeverChange(mWidth - ((mWidth + (mBaseLine >= mHeight - WATER_BOTTOM ? mHeight - WATER_BOTTOM : mBaseLine)) / 5) - ViewUtils.dpToPx(getResources(), 20), mBaseLine >= mHeight - WATER_BOTTOM ? mHeight
					- WATER_BOTTOM : mBaseLine);
		}
	}
	...
	private void refreshBaseLine() {
		if (mPer == 0) {
			...
			mBaseLine = mHeight - (mHeight - mFullLine) * (mPer) - WATER_BOTTOM;
		}
	}
	...
	public void setOnWaterChangedListener(IWaterChangedListener listener) {
		mWaterChangedListener = listener;
		isFirstStart = true;
		isFirstStop = false;
	}
	...
	private void onDrawCanvas(final Canvas canvas) {
		...
		if (null == mBottleBitmap || mBottleBitmap.isRecycled()) {
			createBottleBitmap(R.drawable.home_bottle);
		}
	...
	public void onSensorChanged(SensorEvent event) {
		...
		if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
			...
			if (time - mLastTime > ROTATE_TIME) {
				...
				mAngle = angle;
				if (mWaterChangedListener != null) {
					mWaterChangedListener.onWaterLeansChanged(angle);
					if (angle > 100 && angle < 260 && isFirstStart) {
						mWaterChangedListener.startWater();
						isFirstStop = true;
						isFirstStart = false;
					} else if ((angle < 100 || angle > 260) && isFirstStop) {
						mWaterChangedListener.stopWater();
						isFirstStart = true;
						isFirstStop = false;
					}
				}
	...
	private void createBottleBitmap(int resource) {
		// if (isBack) {
		// Bitmap bitmap = BitmapFactory.decodeResource(getResources(),
		// resource);
		// mBackBottleBitmap = Bitmap.createScaledBitmap(bitmap, (int) mWidth,
		// (int) mHeight, true);
		// } else {
		// Bitmap bitmap = BitmapFactory.decodeResource(getResources(),
		// resource);
		// mBottleBitmap = Bitmap.createScaledBitmap(bitmap, (int) mWidth, (int)
		// mHeight, true);
		// }
		Bitmap bitmap = BitmapFactory.decodeResource(getResources(), resource);
		Canvas canvas;
		mBottleBitmap = Bitmap.createBitmap((int) mWidth, (int) mHeight, Config.ARGB_8888);
		canvas = new Canvas(mBottleBitmap);
