...
public class CanDragProgressView extends View implements OnTouchListener,OnClickListener{
	...
	private int mAbovePaintColor;//画笔颜色
	...
	public int getmAbovePaintColor() {
		return mAbovePaintColor;
	}

	public void setmAbovePaintColor(int mAbovePaintColor) {
		this.mAbovePaintColor = mAbovePaintColor;
		mAbovePaint.setColor(mAbovePaintColor);
		mCenterPaint.setColor(mAbovePaintColor - 0xbb000000);
		invalidate();
	}
	...
	private void init() {
		...
		mBackgroundPaint.setColor(getResources().getColor(R.color.conmmd_line));
		mScalePaint = new Paint();
		mScalePaint.setColor(Color.GRAY);
		mScalePaint.setTextSize(16);
		mCenterPaint = new Paint();
		mCenterPaint.setColor(getContext().getResources().getColor(R.color.demo_title2));
		mAbovePaint = new Paint();
		mAbovePaint.setColor(getContext().getResources().getColor(R.color.demo_title6));
		mAbovePaint.setTextSize(45);
		mDragClickPaint = new Paint();
		mDragClickPaint.setColor(getContext().getResources().getColor(R.color.demo_title7));
...
//		Log.i("progress", mBgProgressWidth+"**"+mProgressHeight+"**"+mStartY+"**"+getFontHeight(mScalePaint));
	...
	private void drawDragClick(Canvas canvas) {
		mDragClickPaint.setColor(getResources().getColor(R.color.conmmd_textcolor_s));
...
//		Log.i("progress", "onTouch"+mAboveProgressWidth);
		if (event.getY() <  mStartY + mProgressRealHeight/2 + 2*mDragClickPaintRadius && event.getY() >  mStartY + mProgressRealHeight/2 - 2*mDragClickPaintRadius 
//				&& event.getX() > mAboveProgressWidth - mDragClickPaintRadius && event.getX() < mAboveProgressWidth + mDragClickPaintRadius
				) {
			isMoving = true;
